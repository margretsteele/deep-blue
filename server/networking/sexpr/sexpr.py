...
class SExprReader(AbstractFilter):
  ...
  def illegal_close_paren(self, i):
    raise SExprIllegalClosingParenError("Ignore a close parenthesis: %d" % i)
    return
  # called if it reaches the end-of-file while the stack is not empty.
  def premature_eof(self, i, x):
    raise SExprPrematureEOFError("Premature end of file: %d parens left, partial=%s" % (i, x))
    return
...
def sexpr2str(e):
  ...
  if not isinstance(e, list):
    if isinstance(e, str):
        ...
        return sexpr2str(list(e))
    elif isinstance(e, float):
        #round to 6 decimal places
        return "%.6f" %e
    else :
        #this should work for ints and floats at least
        return repr(e)
