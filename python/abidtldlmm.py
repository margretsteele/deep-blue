...
def abidtldlmm(board, maxLimit, qlimit, deadline):
  '''
...
def dlminimax(limit, qlimit, b, selector, deadline, first, second):
  '''
  Then calls its sell recursively until the depth or time limit is reached 
...
def timeHeur(ai):
  '''
  Gives each move a set amount of time 
  '''
...
def quieSearch(child, qlimit, parentHeur):
  if (child[1].heruisticGen(True) - parentHeur) >= 3 and qlimit > 0:
    return True
  return False
