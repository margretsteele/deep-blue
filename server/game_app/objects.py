...
class Piece:
  ...
  def move(self, file, rank, type):
    ...
    if self.game.noLegalMoves(self.owner^1):
      if self.game.inCheck(self.owner^1):
        self.game.declareWinner(self.game.players[self.owner],("White" if self.owner == 0 else "Black") + " Wins by Checkmate")
      else:
        self.game.declareDraw("No legal moves availible for " + ("White" if self.owner == 1 else "Black") + ", Stalemate!")
  ...
  def makeMove(self,file,rank,type):
    ...
    if verifyVal is True:
      if self.hasMoved == 0:
        self.hasMoved = 1
        if self.type == ord('P') and abs(self.rank-rank) == 2:
          self.hasMoved = 2
      for piece in [i for i in self.game.objects.values() if isinstance(i, Piece)] :
        if piece.rank == rank and piece.file == file:
          ...
          targetPiece.rank = -1
          targetPiece.file = -1
        if piece.id != self.id and piece.hasMoved == 2:
          piece.hasMoved = 1
      if self.file-file == 2 and self.type == ord('K'):
        ...
        rook.file = file+1
      if self.type == ord('K') and self.file-file == -2:
        if self.game.inCheck(self.owner):
          return ("White" if self.game.playerID == 0 else "Black") + " Attempted to Castle out of Check!"
      ...
      if self.type == ord('P') and rank == 1 + 7 * (self.owner^1):
        ...
        self.type = type
        promotion = True
      if self.type == ord('P') and board[rank-1][file-1] is None and self.file != file:
        #EnPassant Code
        for piece in [i for i in self.game.objects.values() if isinstance(i,Piece)]:
      ...
      if self.game.inCheck(self.owner):
        ...
        return ("White" if self.game.playerID == 0 else "Black") + " Attempted to make a Move which ended with itself in check!"
    else:
      return "type: "+chr(self.type)+" Rank: "+repr(self.rank)+" to "+repr(rank)+", File: "+repr(self.file)+" to "+repr(file)+" "+verifyVal
    #removes taken piece and updates the turns to stalemate
    if self.type == ord('P') or targetPiece is not None or promotion:
  ...
  def verifyMove(self,file,rank,type):
    ...
    if self.file < 1 or self.file > 8 or self.rank < 1 or self.rank > 8:
      ...
      return ("White" if self.game.playerID == 0 else "Black") + " Attempted to make a move which did not move any piece!"
        ...
        return ("White" if self.game.playerID == 0 else "Black") + " Tried to move a piece that belonged to " + ("White" if self.game.playerID == 1 else "Black")
    ...
    #Move logic for a pawn
      ...
      if board[rank-1][file-1] is None:
        ...
        if self.file != file:
          ...
          and board[rank+2*(self.owner-1)][file-1].type == ord('P') \
          and board[rank+2*(self.owner-1)][file-1].owner != self.owner \
          and rank - self.rank + 2*self.owner == 1 \
          and board[rank+2*(self.owner-1)][file-1].hasMoved == 2):
            return  ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a pawn into an empty square which was not directly ahead when said move was not a legal En Passant"
        #checks for double move after the first
        elif rank - self.rank + 4*self.owner == 2:
          if self.hasMoved == 0:
            canMove = self.checkRank(rank)
          else:
            return ("White" if self.game.playerID == 0 else "Black") + " Attempted to double move a pawn after it had already moved"
        #checks for regular move
        elif rank - self.rank + 2*self.owner != 1:
          return ("White" if self.game.playerID == 0 else "Black") + " Attempted to make an illegal move with a Pawn"
      elif abs(self.file-file) != 1 or (rank - self.rank + 2*self.owner != 1):
        return ("White" if self.game.playerID == 0 else "Black") + " Attempted to take a piece with a Pawn in a manner that Pawns cannot"
      if rank == 1 + 7 * (self.owner^1):
        if type != ord('R') and type != ord('N') and type != ord('B') and type != ord('Q'):
          return ("White" if self.game.playerID == 0 else "Black") + " Attempted to move a Pawn into the far row without providing a valid Pawn Upgrade Identifier, instead providing " + chr(type)
      ...
      if (not (self.hasMoved == 1)) and (self.rank == rank) and (abs(self.file-file) == 2):
        if file > self.file:
          ...
          if board[rank-1][7] is None:
            ...
            if board[rank-1][7].owner != self.owner or board[rank-1][7].type != ord('R') or board[rank-1][7].hasMoved != 0:
          ...
          for i in range(file-2, self.file-1):
            ...
            if board[rank-1][0] is None:
              ...
              if board[rank-1][0].owner != self.owner or board[rank-1][0].type != ord('R') or board[rank-1][0].hasMoved != 0:
                return ("White" if self.game.playerID == 0 else "Black") + " Attempted to castle, but the Rook in question had already moved"
     #normal move logic
